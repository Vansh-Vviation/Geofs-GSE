(function () {
    ...
    function spawnCateringTruck() {
        const heading = parseFloat(document.getElementById("headingInputEuro").value) || 0;
        const lateralOffset = (parseFloat(document.getElementById("lateralOffsetInputEuro").value) || 15) * 0.000009;
        const forwardOffset = (parseFloat(document.getElementById("forwardOffsetInputEuro").value) || 0) * 0.000009;
        const aircraftPos = geofs.aircraft.instance.llaLocation;
        const adjustedAlt = aircraftPos[2] - 6.2;
        ...
        euroCaterEntities.push(truck);
    }

    function updateCubeHeight(height) {
        if (euroCaterEntities.length === 0) return;
        const truck = euroCaterEntities[euroCaterEntities.length - 1];
    ...
    function despawnCateringTruck() {
        if (euroCaterEntities.length === 0) return;
        const truck = euroCaterEntities.pop();
        viewer.entities.remove(truck);
    }

    // GUI
    const gui = document.createElement("div");
    gui.id = "euroCaterGuiContainer";
    gui.style = "position:absolute;top:10px;right:10px;background:rgba(0,0,0,0.7);padding:10px;border-radius:5px;color:white;z-index:9999;";

    gui.innerHTML = `
        <h4>Euro Catering Control</h4>
        Heading:<br/><input id="headingInputEuro" type="number" value="0"><br/>
        Lateral Offset (m):<br/><input id="lateralOffsetInputEuro" type="number" value="15"><br/>
        Forward Offset (m):<br/><input id="forwardOffsetInputEuro" type="number" value="0"><br/>
        Cube Height:<br/><input id="heightSliderEuro" type="range" min="0" max="${maxHeight}" step="0.1" value="0"><span id="heightValueEuro">0</span>m<br/><br/>
        <button id="spawnEuroTruck">Spawn</button>
        <button id="despawnEuroTruck">Despawn</button>
    `;

    document.body.appendChild(gui);

    document.getElementById("heightSliderEuro").oninput = (e) => {
        document.getElementById("heightValueEuro").textContent = e.target.value;
        updateCubeHeight(parseFloat(e.target.value));
    };

    document.getElementById("spawnEuroTruck").onclick = spawnCateringTruck;
    document.getElementById("despawnEuroTruck").onclick = despawnCateringTruck;
})();
